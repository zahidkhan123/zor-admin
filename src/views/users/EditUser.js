...
import { useGetDropdownsQuery, useCreateUserMutation } from '../../services/api'
...
const UserProfileForm = () => {
  ...
  const { user, mode } = location.state || { mode: 'add' }
  ...
  useEffect(() => {
    if (mode === 'edit' && user) {
      ...
      setFormData({
        ...
        password: '',
        confirmPassword: '',
        image: user.image || 'profile-image.jpg',
      })
    }
  }, [user, mode])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        navigate('/users', { state: { userAdded: true } })
      } else {
        setSubmitError({ data: { message: response.message || 'Failed to save user' } })
        setShowToast(true)
      }
    } catch (err) {
      setSubmitError(err)
        ...
        <CCardBody>
          <CForm onSubmit={handleSubmit}>
            ...
            <CRow className="justify-content-end mt-4">
              <CCol xs="auto">
                <CButton color="secondary" variant="outline" onClick={() => navigate('/users')}>
                  Cancel
                </CButton>
              </CCol>
              <CCol xs="auto">
